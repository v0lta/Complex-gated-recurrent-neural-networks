...
\centering
\subfigure[]{%
\includegraphics[width=0.4\linewidth]{./img/memory_sGRU_GRU_UNN_LSTM.pdf}}
\qquad
\subfigure[]{%
\includegraphics[width=0.4\linewidth]{./img/adding_sGRU_GRU_UNN_LSTM.pdf}}%
...
\begin{figure}[t!]
    \centering
\subfigure[]{%
\includegraphics[width=0.4\linewidth]{./img/memory_c.pdf}}
\qquad
\subfigure[]{%
\includegraphics[width=0.4\linewidth]{./img/adding_c.pdf}}%
...
%Figure~\ref{fig:complex_results} explores the interplay of activations functions and Stiefel optimization. We conclude that the complex sGRU's performance increases, if the state-transition matrix is kept unitary. On the memory problem the stiefel optimised sGRU using the modRelu activation is able to inherit some of the UNNs favourable properties. \\

% \begin{figure}
%     \centering
%     \includegraphics[width=0.45\linewidth]{./img/adding_r.pdf}
%     \includegraphics[width=0.45\linewidth]{./img/memory_r.pdf}
%     \caption{Results for variations of real gated RNNs on the adding and memory problems for T=250.}
%     \label{fig:real_results}
% \end{figure}
% Results for our real valued sGRU experiments are shown in~\ref{fig:real_results} here the orthogonal networks do generally better with the exception of the tanh on the memory problem. On the fully rms-prop optimised tanh-sGRU does not work very well, because it is initialised with a strong bias towards open gates and does not benefit from orthogonality.    
% Just like in the complex case models using orthogonal state to state transition matrices tend to be more stable.
% We ensured comparable numbers of parameters by setting the real hidden state dimensions equal to $\sqrt{2c^2}$, which leads to a comparable amount of parameters as shown in table~\ref{tab:weights}. Given that the number of weights are comparable we conclude that working in the complex domain leads to better performance on the memory problem. Presumably this is because complex unitary matrices spread their eigenvalues over the entire unit circle, while orthogonal matrices can only have eigenvalues of $\pm 1$. Additionally working in the complex domain brings us an enlargement of the state size with respect to the number of weights required. A complex cell has the the same state dimension as its real counterpart at $n_{hc}/2 = n_{hr}$. The two share the same number of parameters much later at approximately $p_c \approx \sqrt{p_r^2/2}$. Which boots the complex cell's capacity to store information. 
% \begin{table}
% \centering
% \begin{tabular}{|c|c|c|} \hline
%           & complex                      & Real \\ \hline
% Memory     & $n_h=250  \Rightarrow 394501$ Parameters & $n_h=353 \Rightarrow 387603 $ Parameters \\ \hline
% Adding     & $n_h=91  \Rightarrow 51507$ Parameters & $n_h=128 \Rightarrow 50433 $ Parameters \\ \hline
% \end{tabular}
% \caption{Experiment state sizes and total number of weights.}
% \label{tab:weights}
% \end{table}


%To see the impact of norm-preserving state transition matrices on the synthetic tasks, 


% \subsection{sGRU vs. GRU}



% We adopted the Stiefel-manifold-optimisation scheme proposed in~\cite{Wisdom}, to optimise our unitary state to state transition matrix. In this section we asses the effect the unitary state transition matrix has on network performance. The plot on the left of figure~\ref{fig:memoryLSTMUNNCGRU} shows a coupled CGRU with and without Stiefel-Manifold optimisation. We observe that keeping the state transition matrix unitary leads to improved overall performance in the presence of gates. 
% As observed by previous authors unitary state transition matrices are highly beneficial on the memory problem, see figure~\ref{fig:memoryLSTMUNNCGRU} on the right.
